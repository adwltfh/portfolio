interface Job {
  ...
  items: (
    | string
    | { text: string; highlights?: { word: string; type: "bold" | "italic" }[] }
  )[];
}
  ...
  while (rest.length > 0) {
    ...
    if (nextBold < nextItal) {
      ...
      parts.push(
        <b
          key={key++}
          className="bg-resume-butter px-[3px] rounded-[3px] font-bold"
        >
          {word}
        </b>,
      );
      ...
      parts.push(
        <i
          key={key++}
          className="not-italic font-bold text-[13px] bg-resume-pink px-[5px] rounded-[4px]"
        >
          {word}
        </i>,
      );
          ...
          <div key={i} className="relative mb-7 last:mb-0">
            ...
            <div className="bg-resume-paper border-2 border-resume-ink rounded-[18px] px-5 py-[18px] shadow-[4px_4px_0_#b59cf0] transition-transform duration-200 hover:-translate-x-[2px] hover:-translate-y-[2px] hover:shadow-[6px_6px_0_#b59cf0]">
              <div className="flex justify-between items-baseline gap-3 flex-wrap mb-[6px]">
                <h3 className="font-caprasimo text-[20px] m-0 leading-[1.15]">
                  {job.title}
                </h3>
